...
###  二  前期准备 
* 所需组件：1.像素流送插件 ；2.信令和Web服务器  
* 1 - 准备虚幻引擎应用程序  
在虚幻编辑器的主菜单中选择 编辑（Edit） > 插件（Plugins）。在 图像（Graphics） 下找到 像素流送（Pixel Streaming） 插件并勾选 启用（Enabled） 框。
...
通过官方给的样例运行结果如下：  
![image](https://github.com/maolala233/learning_note/blob/main/image/fuse%201.png)  
* **存在问题**：因为我们使用的是pico头显，而本样例支持的头显设备中并不包含pico，因此在设备流式传输过程中存在一些问题，不过经过测试是可以在pico中进入VR模式的。
* _**后续还需继续进行探索调试。**_
